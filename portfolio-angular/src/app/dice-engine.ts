// Copiado tal cual del motor real de la app (physics de cuerpo rígido sobre canvas).
// Sin tipar a propósito: es matemática vectorial genérica, ts-nocheck evita ruido.
/* eslint-disable */
// @ts-nocheck
...
export default class DiceEngine {
  constructor(canvas, options = {}) {
    ...
    this.opts = {
      ...
      fadeIn: 0.22,
      theme: {},
      showBoard: true, // dibuja el tapete; en false el fondo queda transparente
      boardFill: 0.94, // fracción del lienzo que ocupa el área de juego
      faceImagePool: [], // imágenes candidatas para la cara del 6 (logo, dados propios)
      faceTextures: null, // diseño fijo por valor: { 1: {img,sx,sy,sw,sh,inset}, ... }, anula faceImagePool
      ...options
    }
    this.theme = { ...DEFAULT_THEME, ...(this.opts.theme || {}) }

    // Constantes del mundo
    this.Ax = 2.3 // semiancho del área de juego (se recalcula en resize)
    this.Az = 2.3 // semiprofundidad del área de juego (se recalcula en resize)
    this.M = 1
    this.invM = 1
    ...
    this.parts = []
    this.reported = false
    this._time = 0 // reloj acumulado, para el pulso onírico del aura de acierto
    this.h = 0.25
    this.invI = 6
    this.VERT = []
  ...
  rollOne(index, power = 1) {
    ...
    this.reported = false
    const Rx = Math.max(0.2, this.Ax - this.h * 1.5)
    const Rz = Math.max(0.2, this.Az - this.h * 1.5)
    b.start = {
      p: [
        clamp(b.p[0] + rnd() * 0.5, -Rx, Rx),
        1.0 + Math.random() * 0.35,
        clamp(b.p[2] + rnd() * 0.5, -Rz, Rz)
      ],
    ...
    b.phase = 'out'
    b.fT = 0
    b.img = this._pickFaceImage()
    if (this.onRollStart) this.onRollStart({ index })
    return true
  }

  /** Elige al azar una imagen del pool para la cara del 6, o null si no hay ninguna. */
  _pickFaceImage() {
    const pool = this.opts.faceImagePool
    if (!pool || !pool.length) return null
    return pool[Math.floor(Math.random() * pool.length)]
  }

  /** Actualiza el pool de imágenes disponibles para la cara del 6. */
  setFaceImagePool(pool) {
    this.opts.faceImagePool = pool || []
  }

  /**
   * Fija un diseño de caras completo: { 1: {img,sx,sy,sw,sh,inset}, ..., 6: {...} }.
   * Tiene prioridad sobre faceImagePool/los puntos. Pasa null para volver al dado clásico.
   */
  setFaceTextures(faceTextures) {
    this.opts.faceTextures = faceTextures || null
  }
  ...
  roll(power = 1, dx, dz) {
    ...
    const Rx = Math.max(0.2, this.Ax - margin)
    const Rz = Math.max(0.2, this.Az - margin)
    ...
    if (dx !== undefined || dz !== undefined) {
      ...
      bias = [(bx / bl) * Rx * 0.4, (bz / bl) * Rz * 0.4]
    }

    const cw = (2 * Rx) / cols
    const ch = (2 * Rz) / rows
    const spawnZ = this.Az - this.h - 0.05
    const lane = 2 * Math.max(0.1, this.Ax - this.h)

    this.dice.forEach((b, i) => {
      const c = cells[i]
      const tx = clamp(-Rx + cw * ((c % cols) + 0.5) + rnd() * cw * 0.6 + bias[0], -Rx, Rx)
      const tz = clamp(-Rz + ch * (Math.floor(c / cols) + 0.5) + rnd() * ch * 0.6 + bias[1], -Rz, Rz)
  ...
  /**
   * Recalcula el área de juego para que sus bordes coincidan con los del
   * lienzo (sea la tarjeta compacta o la pantalla completa), a partir del
   * ángulo de cámara cenital y el aspecto actual del canvas.
   */
  _computeBounds() {
    const eyeH = VIEWS.top.eye[1]
    const halfVisible = (eyeH / this.fov) * this.opts.boardFill
    const aspect = (this.W && this.H ? this.W / this.H : 1) || 1
    this.Az = Math.max(0.6, halfVisible)
    this.Ax = Math.max(0.6, halfVisible * aspect)
    this.PLANES = [
      { n: [0, 1, 0], d: 0 },
      { n: [1, 0, 0], d: -this.Ax },
      { n: [-1, 0, 0], d: -this.Ax },
  ...
  /* ---------- Interno: motas de acierto ---------- */

  /**
   * Al asentarse un dado en éxito no salta confeti: sueltan unas pocas motas
   * de luz que ascienden despacio y se disuelven, como polvo de sueño.
   */
  _burst(b) {
    const n = 5 + Math.floor(Math.random() * 3)
    const base = [b.p[0], b.p[1] + this.h * 0.7, b.p[2]]
    for (let i = 0; i < n; i++) {
      if (this.parts.length > 120) break
      const a = Math.random() * 6.2832
      const lf = 1.1 + Math.random() * 0.7
      this.parts.push({
        p: [base[0] + Math.cos(a) * this.h * 0.5, base[1], base[2] + Math.sin(a) * this.h * 0.5],
        v: [Math.cos(a) * 0.1, 0.26 + Math.random() * 0.2, Math.sin(a) * 0.1],
  ...
  _marks(dt) {
    ...
    // Las motas flotan hacia arriba con un vaivén suave, sin caer ni rebotar
    for (let i = this.parts.length - 1; i >= 0; i--) {
      const q = this.parts[i]
      q.sway += dt * 1.6
      q.p[0] += (q.v[0] + Math.sin(q.sway) * 0.05) * dt
      q.p[1] += q.v[1] * dt
      q.p[2] += (q.v[2] + Math.cos(q.sway) * 0.05) * dt
      q.v[1] *= 0.995
      q.life -= dt
  ...
  /**
   * Pinta una imagen (o un recorte de un atlas) enmarcada en la cara `f`, en
   * vez de los puntos. `tex` = { img, sx, sy, sw, sh, inset }. sx/sy son
   * opcionales (recorte del atlas); sw/sh son el tamaño de origen a usar;
   * inset es la fracción del semilado de la cara que ocupa la imagen (1 = a
   * ras del borde, <1 deja un marco del color de la cara).
   */
  _drawFaceTexture(f, tex) {
    const ctx = this.ctx
    const h = this.h
    const m = h * (tex.inset !== undefined ? tex.inset : 0.96)
    const p0 = this._proj(this._squash(f.b, sub(sub(f.ctr, mul(f.u, m)), mul(f.t, m))))
    const p1 = this._proj(this._squash(f.b, add(sub(f.ctr, mul(f.u, m)), mul(f.t, m))))
    const p3 = this._proj(this._squash(f.b, sub(add(f.ctr, mul(f.u, m)), mul(f.t, m))))
    const sw = tex.sw
    const sh = tex.sh
    ctx.save()
    ctx.beginPath()
    ...
    if (tex.sx !== undefined) ctx.drawImage(tex.img, tex.sx, tex.sy, sw, sh, 0, 0, sw, sh)
    else ctx.drawImage(tex.img, 0, 0)
    ctx.restore()
  }
  ...
  _draw() {
    ...
    if (this.opts.showBoard) {
      const c0 = this._proj([-Ax, 0, -Az])
      const c1 = this._proj([Ax, 0, -Az])
      const c2 = this._proj([Ax, 0, Az])
      const c3 = this._proj([-Ax, 0, Az])
      ...
      const i0 = this._proj([-Ax * 0.9, 0, -Az * 0.9])
      const i1 = this._proj([Ax * 0.9, 0, -Az * 0.9])
      const i2 = this._proj([Ax * 0.9, 0, Az * 0.9])
      const i3 = this._proj([-Ax * 0.9, 0, Az * 0.9])
    ...
    for (const b of this.dice) {
      ...
      // Aura de acierto: un halo de estrella en azul eléctrico que respira y
      // gira despacio sobre la oscuridad, como un aura de santidad hostil
      if (b.hl > 0.02) {
        const pulse = 0.55 + 0.45 * Math.sin(this._time * 1.4 + b.auraPhase)
        const baseR = this._pxScale(sc[2]) * h
        const glowR = baseR * 2.8
        const cols = this.theme.confetti

        ctx.save()
        ctx.globalCompositeOperation = 'lighter'

        // resplandor de fondo, como un vacío que se abre bajo el dado
        const grad = ctx.createRadialGradient(sc[0], sc[1], 0, sc[0], sc[1], glowR)
        grad.addColorStop(0, cols[0])
        grad.addColorStop(0.55, cols[2] || cols[0])
        grad.addColorStop(1, 'transparent')
        ctx.globalAlpha = b.hl * pulse * 0.45 * b.alpha
        ctx.fillStyle = grad
        ctx.beginPath()
        ctx.arc(sc[0], sc[1], glowR, 0, 6.2832)
        ctx.fill()

        // rayos de estrella, girando despacio: un apunte discreto delante
        // del dado, el protagonismo lo lleva el resplandor de fondo
        const rays = 8
        const rot = this._time * 0.3 + b.auraPhase
        ctx.globalAlpha = b.hl * pulse * 0.22 * b.alpha
        ctx.strokeStyle = cols[1] || cols[0]
        ctx.lineWidth = Math.max(0.6, baseR * 0.04)
        ctx.lineCap = 'round'
        for (let i = 0; i < rays; i++) {
          const an = rot + (i / rays) * 6.2832
          const rIn = baseR * 1.3
          const rOut = glowR * (i % 2 === 0 ? 0.7 : 0.42)
          ctx.beginPath()
          ctx.moveTo(sc[0] + Math.cos(an) * rIn, sc[1] + Math.sin(an) * rIn)
          ctx.lineTo(sc[0] + Math.cos(an) * rOut, sc[1] + Math.sin(an) * rOut)
          ctx.stroke()
        }
        ctx.restore()
      }
    ...
    for (const f of list) {
      ...
      // Diseño fijo (p.ej. gótico): misma imagen para esa cara en todos los dados
      const atlas = this.opts.faceTextures && this.opts.faceTextures[f.val]
      if (atlas && atlas.img && atlas.img.complete && atlas.img.naturalWidth) {
        this._drawFaceTexture(f, atlas)
        continue
      }

      // Logo del estudio o dado propio del jugador, sólo en la cara del 6
      const legacyImg = f.val === 6 ? f.b.img : null
      if (legacyImg && legacyImg.complete && legacyImg.naturalWidth) {
        this._drawFaceTexture(f, {
          img: legacyImg,
          sw: legacyImg.naturalWidth,
          sh: legacyImg.naturalHeight,
          inset: 0.8
        })
        continue
      }
    ...
    // Motas de acierto: estrellas diminutas y parpadeantes que ascienden
    // despacio, varias tonalidades de azul y violeta, como polvo del espacio
    for (const q of this.parts) {
      const sp = this._proj(q.p)
      const t = Math.max(0, Math.min(1, q.life / q.max))
      const twinkle = 0.5 + 0.5 * Math.sin(this._time * 7 + q.sway * 3)
      const rr = Math.max(2, q.r * this._pxScale(sp[2]) * (0.7 + (1 - t) * 0.5))
      const alpha = Math.sin(t * Math.PI) * (0.5 + 0.5 * twinkle)
      ctx.save()
      ctx.globalCompositeOperation = 'lighter'
      ctx.globalAlpha = alpha
      const grad = ctx.createRadialGradient(sp[0], sp[1], 0, sp[0], sp[1], rr)
      grad.addColorStop(0, q.col)
      grad.addColorStop(1, 'transparent')
      ctx.fillStyle = grad
      ctx.beginPath()
      ctx.arc(sp[0], sp[1], rr, 0, 6.2832)
      ctx.fill()
      // destello en cruz, apenas insinuado: sólo un apunte de que es una
      // estrellita y no una bola, sin robarle protagonismo al resplandor
      ctx.globalAlpha = alpha * 0.5
      ctx.strokeStyle = q.col
      ctx.lineWidth = Math.max(0.5, rr * 0.1)
      ctx.beginPath()
      ctx.moveTo(sp[0] - rr * 1.3, sp[1])
      ctx.lineTo(sp[0] + rr * 1.3, sp[1])
      ctx.moveTo(sp[0], sp[1] - rr * 1.3)
      ctx.lineTo(sp[0], sp[1] + rr * 1.3)
      ctx.stroke()
      ctx.restore()
    }
    ctx.globalAlpha = 1
